...
cat << "EOF"
  ...
  ''  ,;@@@@@'  ;@@@@; ''    ;;@@@@@;;;;
    ...
    ;;@@@@@@;           , ';;;@@@@@@@@;;;.
     '';@@@@@,.  ,   .   ',;;;@@@@@@;;;;;;
        .   '';;;;;;;;;,;;;;@@@@@;;' ,__;___   _ ___________  _____
          ''..,,     '''' '; ; | |_|_   _   \ | | ____/  ___|/  ___|
               ''''''::'''| |  | | | | | |   \| |  __|\___  \\___  \
                          | |/\| | | | | | |\   | |______/  /___/  /
                          |__/\__|_| |_| |_| \_____________//_____/
...
if [[ $answer5 =~ ^(yes|Yes|y|Y) ]]
  then
    ...
    cat live-hosts.txt && cat live-hosts.txt | wc -l
...
/opt/eyewitness/EyeWitness.py -x /root/Desktop/$date/nmap/$date.xml --timeout 90 --threads 10 --no-prompt --web --max-retries 1 -d /root/Desktop/$date/eyewitness > /dev/null

cd ../eyewitness/
firefox report.html& 

cd /root/Desktop/$date/nmap
gowitness nmap --nmap-file $date.xml
gowitness report generate

#####################################################################################################
#                                     Run additional nmap ssl scan                                  #
#####################################################################################################
...
if [[ $answer7 =~ ^(yes|Yes|y|Y) ]]
  then
    ...
    xsltproc -o $date-ssl.html /opt/nmap-bootstrap-xsl/nmap-bootstrap.xsl $date-ssl.xml
